...
import { useFormData } from "utils/DataContext";
...
const Karkit = () => {
  ...
  const countCharactersNFT = useMemo(() => {
    if (isLoading) return [];
    // const kartKitIds = getRacingTeams.map((item) => item.kartKitIds.toString());
    const kartKitIds = getRacingTeams.flatMap((item) => item.kartKitIds.map(String));
    const flattenedKartKitIds = kartKitIds.join(",").split(",");

    return getWalletNFTs?.filter(
      (item) => item?.typeNFt !== "NFT" && !flattenedKartKitIds.includes(item.id.toString())
    );
  ...
  const handleCheckboxChange = (event, nft) => {
    ...
    if (isChecked) {
      setFormData((prevItems) => [...prevItems, { ...nft, nftid: item, checked: isChecked }]);
    } else {
      setFormData((prevItems) => prevItems.filter((obj) => obj.nftid !== item));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        <h1 className="text-2xl font-semibold whitespace-nowrap">Karkits</h1>
      </div>
        ...
        <>
          <div className="p-5 rounded-lg bg-primary-pe flex flex-row justify-between items-start ">
            <div className="flex flex-col items-start gap-1">
              <span className="text-xl">Filter Karkits By Rarity</span>
          ...
          <div className=" mt-2 container mx-auto px-4 ">
            {listCharacters.length ? (
              <>
                {" "}
                <div>
                  <h2 className="mb-5 pl-2 text-lg font-medium text-gray-900 dark:text-back">Selet KarKit</h2>
                </div>{" "}
                    ...
                    {listCharacters?.map((item, key) => (
                      <div key={key}>
                        <input
                          ...
                          checked={formData.checked} // Verifica si el item está en formData2
                          onChange={(event) => {
